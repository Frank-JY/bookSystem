...
<%@ include file="head.jsp" %>
	...
	if(null != session.getAttribute("borrowMap")) {
		...
		Map<StudentBorrow, String> borrowMap = (Map<StudentBorrow, String>)session.getAttribute("borrowMap");
		Set<Map.Entry<StudentBorrow, String>> borrowSet = borrowMap.entrySet();
	...
	%> 
		<tr>
			<td><%=i++ %></td>
			<td><a href="BookDetailsServleet?bookId=<%=borrow.getBookId() %>&tag=9"><%=borrow.getBookId() %></a></td>
			<td><a href="BookDetailsServleet?bookId=<%=borrow.getBookId() %>&tag=9"><%=bookName %></a></td>
