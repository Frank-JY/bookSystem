...
<%@ include file="../head.jsp" %>
	...
	<center>
      <table  height="60" cellpadding="0" cellspacing="0" border="0">
       ...
       <table style="font-size: 20px;">
		...
		<br>
		<a href="../favourite/favourite1.do?bookId=<%=bookId %>"><img  src="../image/upload/image/collect.jpg"></a>
		<a href="../borrow/borrow.do?bookId=<%=bookId %>"><img  src="../image/upload/image/borrow.jpg"></a>
		<%	
			String tag = "";												//返回标签
   ...
   %>
	...
	%>
		<div class="del">
			<a href="../comment/deleteComment.do?id=<%=comment.getId() %>&bookId=<%=bookId %>&tag=<%=tag %>" onclick="return del();">
	...
	%>
			<div class="commentphoto">
				<a href="../user/lookUser.do?userName=<%=comment.getUserName() %>&messageTag=1">
					<img alt="头像" src="../image/upload/<%=comment.getPhoto() %>" width="100px" height="100px;" title="头像">
				</a>
			</div>
   		</div>
   ...
   <center>
		...
		<a href="../borrow/toBookDetails.do?page=1&tag=<%=tag %>&k=<%=k %>">首页</a>
		<a href="../borrow/toBookDetails.do?page=<%=(currentPage - 1)%>&tag=<%=tag %>">上页</a>
		<a href="../borrow/toBookDetails.do?page=<%=(currentPage + 1)%>&tag=<%=tag %>">下页</a>
		<a href="../borrow/toBookDetails.do?page=<%=pUtil.getPageCount()%>&tag=<%=tag %>">末页</a>
	</center>
