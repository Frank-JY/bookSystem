		...
		<form action="search.do" method="post">
			<table>
				<tr>
					...
					<td><a href="toCondition1.do?value=1">点击进入条件查询</a></td>
				</tr>
			</table>
		</form>
	</center>
      ...
      <tr>
        <td> 
          ...
          <!-- 商品循环开始 -->
           ...
           <c:if test="${searchBooksList[0] == null }">
           		<center><font color="blue" size="5">无相关记录！</font></center>
           </c:if>
           <c:set value="0" var="i" />
  
       <c:forEach items="${searchBooksList }" var="searchBook">
          <div id="search" align="center">
             <dl>
               <dt class="picture">
               <c:set value="${i+1 }" var="i" />
               No${i }
                 <a href="bookDetails.do?bookId=${searchBook.bookId}&tag=8"> <img src="../image/upload/${searchBook.photo}" width="130" height="180" border="1"/></a>
               </dt>
               	<dt>
               	书名:${searchBook.bookName}<br>
               	作者:${searchBook.author}<br>
               	状态:${searchBook.status}<br>
               </dt>
             </dl>
          </div>         
          <!-- 商品循环结束 -->
