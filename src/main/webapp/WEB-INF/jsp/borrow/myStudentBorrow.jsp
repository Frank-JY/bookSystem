...
<%@ include file="../head.jsp" %>
	...
	if(null != session.getAttribute("myBorrowList") && session.getAttribute("identity").equals("学生")) {
		@SuppressWarnings("unchecked")
		List<StudentBorrow> myBorrowList = (List<StudentBorrow>)session.getAttribute("myBorrowList");
	...
	%> 
		<tr>
			<td><%=i++ %></td>
			<td><a href="../book/bookDetails.do?bookId=<%=borrow.getBookId() %>&tag=2"><%=borrow.getBookId() %></a></td>
			<td><a href="../book/bookDetails.do?bookId=<%=borrow.getBookId() %>&tag=2"><%=bookName %></a></td>
		...
		%>
			...
			<td><center><a href="backBook.do?id=<%=borrow.getId() %>&identity=student" onclick="return back();">归还图书</a></center></td>
		<%
