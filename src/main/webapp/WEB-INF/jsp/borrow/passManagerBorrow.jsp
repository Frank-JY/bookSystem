...
<%@ include file="../head.jsp" %>
	...
	<c:set value="0" var="i" />
	<c:forEach items="${maBorrowList }" var="borrow">
		<tr>
			<c:set value="${i+1 }" var="i" />
			<td>${i }</td>
			<td><a href="../book/bookDetails.do?bookId=${borrow.bookId}&tag=5">${borrow.bookId}</a></td>
			<td><a href="../book/bookDetails.do?bookId=${borrow.bookId}&tag=5">${borrow.bookName}</a></td>
			...
			<td>${borrow.status }</td>
			<td>
				<a href="passBorrowAction.do?action=pass&id=${borrow.id}&identity=manager" onclick="return pass();">同意</a>&nbsp;&nbsp;
				<a href="passBorrowAction.do?action=reject&id=${borrow.id}&identity=manager" onclick="return reject();">不同意</a>
			</td>
		</tr>
	</c:forEach>
